...
def create_url_table(db_structure,save_db,tb_name,ip_address,db_structure_url):
    ...
    if res:
        ...
        if get_db_creds:
            ...
            conn = psycopg2.connect("dbname='{}' user='{}' host='{}' password='{}'".format(dbname,get_db_creds[1],get_db_creds[0],get_db_creds[2]))
            cur = conn.cursor(cursor_factory=DictCursor)
            cur.execute("""CREATE TABLE IF NOT EXISTS {}_queue ( id serial, {} ) WITH (OIDS=FALSE)""".format(tb_name,','.join(table_fields)))
            conn.commit()
            conn.close()
...
def update_db(db_user_pass,tb_name,product_url,colum_name,day_limits):
    try:
        conn = psycopg2.connect("dbname='{}' user='{}' host='{}' password='{}'".format(config_dbname,db_user_pass[1],db_user_pass[0],db_user_pass[2]))
        cur = conn.cursor(cursor_factory=DictCursor)
        cur.execute("update "+tb_name+"_queue set scrapped='1' where "+colum_name+"=%s and day_limits=%s",(product_url,day_limits))
        conn.commit()
        conn.close()
...
def get_location(db_user_pass,tb_name,colum_name):
    ...
    conn = psycopg2.connect("dbname='{}' user='{}' host='{}' password='{}'".format(config_dbname,db_user_pass[1],db_user_pass[0],db_user_pass[2]))
    cur = conn.cursor(cursor_factory=DictCursor)
    cur.execute("select "+colum_name+",day_limits from {}_queue where scrapped='0'  order by id asc;".format(tb_name))
...
def from_queue_locations(db_user_pass,tb_name):
    ...
    cur.execute("select * from "+tb_name+"_queue")
    res=cur.fetchall()
    conn.commit()
    conn.close()
    return res
...
def spider_jobs(db_user_pass,website_name,job_name,status,pickup_date,completed_date,created_date,error_log):
    try:
        ...
        cur = conn.cursor(cursor_factory=DictCursor)
            #insert_url = ''' INSERT INTO {}({}) VALUES({})''' .format(tb_name,','.join(db_variable),','.join(input_columns))
        cur.execute("insert into spider_jobs (id,website_name,job_name,status,pickup_date,completed_date,created_date,error_log) values (DEFAULT,%s,%s,%s,%s,%s,%s,%s)",(website_name,job_name,status,pickup_date,completed_date,created_date,error_log))
        conn.commit()
        conn.close()
